...
- (NSInteger)numberOfSectionsInCollectionView:(VMCollectionView *)collectionView {
    return self.sectionViewModels.count;
}

- (NSInteger)collectionView:(VMCollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return [self.sectionViewModels[section] count];
}
#if TARGET_OS_IPHONE
- (VMCollectionViewCell *)collectionView:(VMCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
#elif TARGET_OS_MAC
- (VMCollectionViewCell *)collectionView:(VMCollectionView *)collectionView itemForRepresentedObjectAtIndexPath:(NSIndexPath *)indexPath
...
- (VMCollectionReusableView *)collectionView:(VMCollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    if ([kind isEqualToString:VMCollectionElementKindSectionHeader]) {
    ...
    if ([kind isEqualToString:VMCollectionElementKindSectionFooter]) {
        SectionViewModel *sectionViewModel = self.sectionViewModels[indexPath.section];
